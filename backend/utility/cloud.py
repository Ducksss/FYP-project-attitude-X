...
def uploadFile(uploaded_file,bounding_vid=0):
    ...
    if bounding_vid == 1:
        file = drive.CreateFile({'title':f'{uploaded_file.split("/")[2].split(".")[0]}','parents': [{'id': f'{bound_folderID}'}]})
    else:
...
def downloadFile(file_url):
    file_list = drive.ListFile({'q': "'1VW3jHXlaUd_tpCWvA3xG_vv3HX5xTHPK' in parents and trashed=false"}).GetList()
    for file in file_list:
        if file['embedLink'] == file_url:
            file.GetContentFile('./records/temp_video.mp4')
            return file['title']
